...
def read_rosbag_to_one_csv(bag_path: str, topic_names: List[str], out_dirpath: str):
...
def main():
    # Directory path of rosbag2 (manually specified)
    bag_fpath_list = glob.glob(r'./rosbag2_out/*')
    bag_fpath_list.sort()
    bag_path = bag_fpath_list[-1]

    print('Input rosbag: {}'.format(bag_path))

    out_dirpath = './result_csv'
    if not os.path.exists(out_dirpath):
        os.makedirs(out_dirpath, exist_ok=True)
    
    topic_list = []
    read_rosbag_to_one_csv(bag_path, topic_list, out_dirpath)
